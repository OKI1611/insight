...
(function(){
  ...
  var DEFAULT_MENU = [
    ...
    { label:'성경', href:'bible.html', children:[
      ...
      { label:'1611년 킹제임스 서문', href:'kjv-preface.html' }
    ]},
    ...
    { label:'질의응답', href:'qna.html', children:[
      ...
      { label:'수강생 질문방', href:'qna-board.html' },
  ...
  function buildQuickRail(s){
    ...
    function syncRailTop(){
      var h = 0;
      try{
        // 스크롤 전에는 헤더가 아직 상단에 붙어 있지 않으므로 '현재 위치'가 아니라
        // 'top:0 으로 붙게 되어 있는가(css top)'로 판별하고 높이만 가져온다.
        var cand = document.querySelectorAll('header,[data-sticky-header]');
        for(var i=0;i<cand.length;i++){
          var cs = getComputedStyle(cand[i]);
          if(cs.position!=='sticky' && cs.position!=='fixed') continue;
          if(cs.display==='none') continue;
          var ct = parseFloat(cs.top);
          if(isNaN(ct) || ct>2) continue;                        // 상단 고정이 아닌 것 제외
          var b = cand[i].getBoundingClientRect();
          if(b.width < innerWidth*0.7) continue;                 // 전체폭 헤더만
          if(b.height < 30 || b.height > innerHeight*0.6) continue;  // 전면 오버레이 등 제외
