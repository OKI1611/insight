...
(function(){
  ...
  function toast(m){ var t=document.getElementById('cmsToast'); if(!t){ t=document.createElement('div'); t.id='cmsToast'; t.style.cssText='position:fixed;bottom:84px;left:50%;transform:translateX(-50%);z-index:120;background:#2b3440;color:#fff;padding:10px 18px;border-radius:999px;font-size:13px;box-shadow:0 8px 24px rgba(0,0,0,.2)'; document.body.appendChild(t);} t.textContent=m; t.style.display=''; clearTimeout(t._t); t._t=setTimeout(function(){t.style.display='none';},2400); }
  ...
  ready(function(){
    ...
    bar.style.cssText='position:fixed;top:8px;left:50%;transform:translateX(-50%);z-index:120;display:flex;gap:8px;align-items:center;background:#2b3440;color:#fff;padding:7px 12px;border-radius:999px;box-shadow:0 8px 24px rgba(0,0,0,.25);font-size:13px';
    bar.innerHTML='<span style="opacity:.65">관리자 ·</span>'
      +'<button id="cmsToggle" style="background:#6f8f6a;color:#fff;border:0;padding:6px 13px;border-radius:999px;font-weight:700;cursor:pointer">✏️ 글 편집</button>'
      +'<button id="cmsSave" style="display:none;background:#fff;color:#2b3440;border:0;padding:6px 13px;border-radius:999px;font-weight:700;cursor:pointer">💾 저장</button>'
  ...
  function setEdit(on){
    ...
    document.querySelectorAll('[data-cms]').forEach(function(el){
      ...
      el.style.outline=on?'2px dashed #6f8f6a':'';
