...
(function(){
  ...
  try{
    if(document.body && !document.getElementById('biblyFab')){
      ...
      w.innerHTML =
        ...
        + '<span style="font-size:11px" class="text-neutral-900/45 px-1 select-none">글자</span>'
        + '<button onclick="__biblyFont(-1)" aria-label="글자 작게" class="rounded-full hover:bg-ink/5 text-neutral-900/60" style="width:34px;height:34px;font-size:13px">가&minus;</button>'
        ...
        + '<button id="installBtn" onclick="__biblyOpenInstall()" aria-label="앱 설치" class="inline-flex items-center gap-1.5 bg-ink text-white font-bold rounded-full shadow-xl hover:bg-navy transition" style="display:none;padding:11px 18px;font-size:15px"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect x="6" y="2.5" width="12" height="19" rx="2.5"/><path d="M12 7v7M9.2 11.2 12 14l2.8-2.8"/><path d="M10 18.5h4"/></svg>앱 설치</button>'
        ...
        + '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" style="margin-left:16px"><path d="M21 11.5a8.5 8.5 0 0 1-12.4 7.6L3 21l1.9-5.6A8.5 8.5 0 1 1 21 11.5z"/><path d="M9.5 9.2a2.5 2.5 0 0 1 4.9.6c0 1.6-2.4 2-2.4 3.2"/><path d="M12 16.5h.01"/></svg>'
      ...
      if(!document.getElementById('installModal')){
        ...
        im.innerHTML =
          ...
          + '<button onclick="__biblyPwaInstall()" class="w-full py-3 rounded-xl font-semibold text-sm transition" style="background:#213a6b;color:#ffffff">지금 설치하기</button></div>'
          + '<div id="installAndroidManual" style="display:none"><ol class="text-sm text-gray-700 space-y-3">'
          + '<li class="flex gap-2"><span style="color:#171717;font-weight:700">1.</span><span>브라우저 오른쪽 위 <b>⋮ 메뉴</b>(점 3개)를 누르세요.</span></li>'
  ...
  function render(){
    ...
    var support = '';  // 후원 진입은 상단바·헤더 배너로 일원화(2026-09-06)
    if(loggedIn){
      el.innerHTML =
