...
(function(){
  ...
  window.biblyAccessBanner = function(st){
    if(!st) return '';
    if(st.certExpired && st.paidTier === 0){   // 인증과정 수강기간(12개월) 만료 → 재등록 안내
      return '<div class="bg-ink text-paper rounded-2xl px-5 py-4 flex flex-col sm:flex-row sm:items-center gap-3">'
        + '<div class="flex-1 text-sm leading-relaxed"><b>인증 과정 수강기간(1년)이 종료</b>되었어요. 시험·1:1 질의응답·교재 등 유료 혜택을 이어가시려면 재등록(연장)이 필요합니다. 이미 취득한 학점·수료는 보존됩니다.</div>'
